...
from sirtaGSIGHI import makeDatasetGSIfeatureGHI
...
def trainSingleGPU(args):
    ...
    # Make Dataset
    trainDataset = makeDatasetGSIfeatureGHI(args.featureDir, trainStartDate, trainEndDate)
    # Create DataLoader
    ...
    if useValSet:
        ...
        scalingParams = trainDataset.getScalingParams()
        valDataset = makeDatasetGSIfeatureGHI(args.featureDir, valStartDate, valEndDate, normalization_params=normalizationParams, scaling_params=scalingParams)
        # Create DataLoader
    ...
    summary(model, [(args.batchSize,4*4), (args.batchSize,4*4), (args.batchSize,1), (args.batchSize,1), (args.batchSize,1)])
    ### Define loss function and optimizer
    criterion = nn.MSELoss().to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lrDens)
    ...
    for epoch in range(startEpoch, args.numEpochs):
        ...
        for i,batch in progress_bar:
            ...
            targets = batch['target'].float().view(-1, 1).to(device)
            # Forward pass
            outputs = model(cloudFraction, cloudClass, csm, sza=sza, saa=saa)
...
def trainMultiGPU(gpu, args):
    ...
    if useValSet:
        ...
        # Create DataLoader
        valDataLoader = DataLoader(valDataset, batch_size=args.batchSize,
    ...
    ### Print summary of the model
    # summary(model, [(args.batchSize, 3, 256, 256), (args.batchSize, 3)])
    ### Define loss function and optimizer
    criterion = nn.MSELoss().to(device)
    ...
    for epoch in range(startEpoch, args.numEpochs):
        ...
        if useValSet:
            ...
            dist.reduce(avg_validation_loss_tensor, 0)
        ...
        if rank==0 and args.ckptInterval is not None and args.ckptSavePath is not None:
            if (epoch + 1) % args.ckptInterval == 0:
                ...
                print(f'Saved checkpoint at epoch {epoch + 1}')
        # Early Stopping ACROSS ALL GPUs
        early_stopping_decision = torch.tensor(early_stopping_decision, device=device)
        dist.all_reduce(early_stopping_decision, op=dist.ReduceOp.SUM)
        # If any GPU has decided to stop early, break the training loop
...
if __name__=="__main__":
    global_start_date = "20200101"
    global_end_date = "20210630"
    validation_split_fraction = 0.2
    train_start_date_str, train_end_date_str, val_start_date_str, val_end_date_str = generate_train_val_dates(
        global_start_date, global_end_date, validation_split_fraction)
    print(train_start_date_str, train_end_date_str, val_start_date_str, val_end_date_str)